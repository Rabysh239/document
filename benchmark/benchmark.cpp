...
void read_simple(benchmark::State &state) {
  auto allocator = std::pmr::unsynchronized_pool_resource();
  auto doc = gen_doc(1000, &allocator);
  std::string_view int_key{"/count"};
  std::string_view str_key{"/countStr"};
  std::string_view double_key{"/countDouble"};
  std::string_view bool_key{"/countBool"};
  auto f = [
          &doc,
          int_key,
          str_key,
    ...
    doc->get_int(int_key);
    doc->get_string(str_key);
    doc->get_double(double_key);
    doc->get_bool(bool_key);
  };
  for (auto _: state) {
    for (int i = 0; i < state.range(0); ++i) {
      f();
